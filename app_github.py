...
def search():  
    ...
    if search_query == '':
        return render_template('ind.html',prof_data=prof_data,time_taken = round(time.time() - start_time, 5), search_query = search_query, query_method = query_method, index_type = index_type, results_found=results_found)  
...
@app.route('/templates/images/<path:path>')
def send_image(path):
    '''
